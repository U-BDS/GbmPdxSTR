...
myModuleUI <- function(id) {
  ...
  fluidPage(
    fluidRow(
      style = "background-color:#f5f5f5; max-width: 800px; margin-left:
  ...
  moduleServer(
    ...
    function(input, output, session) {
      output$filtered_data <- DT::renderDT({
        ...
        user_df <- process_query(
          query = validate_marker_query(user_df, input_type = "manual_entry"),
      ...
      output$filtered_data_csv <- DT::renderDT({
        ...
        user_query_upload <- process_query(
          query = as.data.frame(user_query_upload),
          include_amel = input$count_amel,
          scoring_algorithm = sub("_.*", "", input$score),
          masters_denominator = sub(".*_", "", input$score)
        )
      ...
      output$download_template <- downloadHandler(
        ...
        content = function(file) {
          write.csv(utils::read.csv("./data/single_query_blank_template.csv"), file, row.names = FALSE)
        }
      )
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$download_template_multi <- downloadHandler(
        ...
        content = function(file) {
          write.csv(utils::read.csv("./data/multi_query_blank_template.csv"), file, row.names = FALSE)
        }
      )
    }
  )
